import { Fingerprint, Check } from 'lucide-react';
import logoUasz from '../../assets/logos/uasz.jpg';

export const AuthLayout = ({ children, title, subtitle }) => {
  const avantages = [
    "CV d'Exception - Créez un profil qui impressionne",
    "Réseau d'Influence - Connectez avec des alumni inspirants",
    "Mentorat Privilégié - Bénéficiez de conseils experts",
    "Opportunités VIP - Accédez à des offres et formations exclusives"
  ];

  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-3">
      {/* Carte globale unique */}
      <div className="flex max-w-6xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
        
        {/* Panneau gauche */}
        <div className="hidden lg:flex lg:w-1/2 bg-gray-50 px-10 py-12 items-center">
          {/* plus de bg-white / border / shadow ici */}
          <div className="w-full">
            {/* En-tête avec logo */}
            <div className="flex flex-col items-center pt-4 pb-0">
              <img
                src={logoUasz}
                alt="Logo UASZ"
                className="h-32 w-32 object-contain mb-3"
              />
              <div className="w-full h-8 bg-emerald-600 rounded-b-2xl mb-10" />
            </div>

            {/* Contenu de la carte */}
            <div className="flex flex-col items-center px-4 pb-4">
              <div className="w-36 h-36 mb-8 relative flex items-center justify-center">
                <div className="absolute inset-0 rounded-2xl border-4 border-emerald-100 bg-emerald-50 flex items-center justify-center" />
                <Fingerprint size={80} className="text-emerald-600 z-10" strokeWidth={1.5} />
                <div className="absolute -top-2 -right-3 w-6 h-6 bg-emerald-600 rounded-full border-4 border-white" />
                <div className="absolute -bottom-3 -left-3 w-6 h-6 bg-emerald-600 rounded-full opacity-50" />
              </div>

              <h3 className="text-2xl font-bold text-gray-900 text-center mb-3">
                Propulsez Votre Carrière avec Notre Communauté Alumni
              </h3>

              <div className="w-full space-y-4 mt-8">
                {avantages.map((avantage) => (
                  <div key={avantage} className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-6 h-6 rounded-full bg-emerald-600 flex items-center justify-center mt-0.5">
                      <Check size={16} className="text-white" strokeWidth={3} />
                    </div>
                    <p className="text-base text-gray-700 flex-1 leading-relaxed">
                      {avantage}
                    </p>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </div>

        {/* Panneau droit - Formulaire */}
        <div className="flex-1 flex items-center justify-center bg-white px-10 py-12">
          <div className="w-full max-w-md">
            {title && (
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-2">{title}</h2>
                {subtitle && (
                  <p className="text-gray-600">{subtitle}</p>
                )}
              </div>
            )}
            {children}
          </div>
        </div>
      </div>
    </div>
  );
};
